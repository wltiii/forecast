...
Install the following tools if you do not already have them:
 ...
 . Create an account at Weather Underground (wundergroun -- http://www.wunderground.com/weather/api/d/docs?d=index)
   and enter the apikey setting in `forecast.yml` and `test-forecast.yml`.
 ...
 . Navigate into the project directory, for exampe [projects home]/forecast.
 ...
 . Once all three of the services have been started -- activity, forecast and
   ...
   `http://localhost:9002/forecast/states/CA/cities/San_Francisco`, being sure
   to use your port settings, if changed.

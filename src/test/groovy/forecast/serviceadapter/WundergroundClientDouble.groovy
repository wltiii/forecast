...
class WundergroundClientDouble extends WundergroundClient {

	JsonSlurper jsonSlurper = new JsonSlurper()
	
	@Override
	def retrieveFromProvider(final String apiPath) {
		Matcher matcher = apiPath =~ "/api/\\S.+/forecast10day/q/(CA|IL|MN)/(Chicago|Minneapolis|San_Francisco)\\.json"
		if (matcher.matches()) {
			jsonSlurper.parseText(tenDayForecast(matcher[0][2]))
			
		}
		else {
			throw new IllegalArgumentException("""API path is invalid. Path is $apiPath. 
Did you run a test that did not match one of the magic values? See the regex expression in
WundergroundClientDouble.retrieveFromProvider(final String apiPath). """)
			
		}
	}
	
	private String tenDayForecast(String city) {
		if (city == "Chicago") {
			chicagoTenDayForecast()
		}
		else if (city == "Minneapolis") {
			minneapolisTenDayForecast()
		}
		else {
			sanFranciscoTenDayForecast()
		}

	}
	
	private String chicagoTenDayForecast() {
		"""{
    "response": {
        "version": "0.1",
	...
	private String minneapolisTenDayForecast() {
		"""{
    "response": {
        "version": "0.1",
        "termsofService": "http://www.wunderground.com/weather/api/d/terms.html",
	...
	private String sanFranciscoTenDayForecast() {
		"""{
    "response": {
        "version": "0.1",
        "termsofService": "http://www.wunderground.com/weather/api/d/terms.html",
